...
import {
  ...
  ref,
  uploadBytes,
  getDownloadURL, // eslint-disable-next-line
  getBytes,
...
export async function getImgPath(img) {
  // Create a child reference
  // eslint-disable-next-line
  // imagesRef now points to 'images'
  let imgUrl = "";
  await getDownloadURL(ref(storage, `anime_img/${img}`))
  .then((url) => {
    console.log(url)  
    imgUrl = url
  })
...
export async function setUserProfilePhoto(uid, file) {
  ...
  const mountainImagesRef = ref(storage, `anime_img/${uid}`);
...
export async function getAnimeInfo(uid) {
  const docRef = doc(db, "anime", uid);
  const docSnap = await getDoc(docRef);
  return docSnap.data();
}
...
export async function getAnimeList() {
  ...
  // const animes = Array.from(querySnapshot.docs);
